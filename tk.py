...
class IntelligentMasonrySystem:
    def __init__(self, master):
        ...
        # ========== 以下变量已注释，不再使用 ==========
        # self.wall_data = []  # 存储从CSV读取的墙体数据
        # self.csv_path = r"C:\Users\25879\Desktop\BIM\wall_coordinates.csv"  # CSV文件路径
        # self.current_wall_index = 0  # 当前选中的墙体索引
        ...
        # 左侧面板 - 控制区域
        left_frame = ttk.Frame(content_frame, padding=10, style='Left.TFrame', width=300)
        ...
        # 连接按钮
        self.connect_btn = ttk.Button(left_frame, text="连接到Revit", command=self.connect_tcp, style='Connect.TButton')
        self.connect_btn.pack(fill=tk.X, pady=(0, 15))
        self.style.configure('Connect.TButton', font=self.default_font)
        
        # 开始砌筑按钮 - 底部
        self.start_btn = ttk.Button(left_frame, text="发送砌筑指令", command=self.start_masonry, style='Start.TButton')
        ...
        # ========== 以下代码已注释，不再使用 ==========
        # # 墙体选择区域
        # self.create_wall_selection(left_frame)
        
        # # 参数设置区域 - 使用LabelFrame包装
        # params_frame = ttk.LabelFrame(left_frame, text="Masonry Parameters", padding=20, style='Params.TLabelframe')
    ...
    def connect_tcp(self):
        try:
            ...
            self.connect_btn.configure(state="normal", text="连接到Revit")
    ...
    def connect_to_revit(self):
        ...
        try:
            ...
            # 恢复按钮状态
            self.master.after(0, lambda: self.connect_btn.configure(state="normal", text="连接到Revit"))
    ...
    def start_masonry(self):
        ...
        try:
            ...
            if not all_bricks:
                messagebox.showwarning("无数据", "请先从Revit接收砖块数据")
                self.log("错误: 没有可用的砖块数据")
                return
            
            # 记录参数到日志
            self.log("开始砌筑任务...")
            self.log(f"共 {len(all_bricks)} 个砖块准备砌筑")
            
            # 计算层数和循环次数
            total_bricks = len(all_bricks)
            layers = (total_bricks + 14) // 15  # 每层平均15块砖（8+7）
            loop_count = layers * 2  # 每层包含两个循环（奇数次和偶数次）
            
            self.log(f"计算结果: 总砖块数={total_bricks}, 层数={layers}, 循环次数={loop_count}")
                    ...
                    while True:
                        ...
                        try:
                            ...
                            while True:
                                ...
                                # 如果收到READY请求，发送坐标数据
                                if "READY" in request and all_bricks:
                                    brick = all_bricks[0]
                                    # 获取8个点坐标
                                    max_point = brick.get('max_point', [0, 0, 0])
                                    min_point = brick.get('min_point', [0, 0, 0])
            ...
            self.log(f"总砖块数: {total_bricks}, 层数: {layers}, 循环次数: {loop_count}")
            self.log("等待机器人连接...")
            
        except Exception as e:
            messagebox.showerror("生成错误", f"生成RAPID指令失败: {str(e)}")
            self.log(f"生成错误: {str(e)}")
